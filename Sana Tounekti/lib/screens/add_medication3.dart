...
class AddMedication3 extends StatefulWidget {
  ...
  const AddMedication3({Key? key, this.targetUserEmail}) : super(key: key);

  @override
  _AddMedication3State createState() => _AddMedication3State();
}

class _AddMedication3State extends State<AddMedication3> with TalkbackScreenMixin {
  ...
  void didChangeDependencies() {
    ...
    speakOnLoad('${translation(context).addMed}. ${translation(context).medTimes}, ${translation(context).medFreq}, ${translation(context).sDate}, ${translation(context).eDate}. ${translation(context).next}');
  }
  ...
  void goToSummaryPage() {
    if (selectedTimes.isEmpty) {
      ...
      if (_medicationEndingDateController.text.isNotEmpty) {
        ...
        if (start.length < 3) {
          _showSnackBar('Invalid start date');
          return;
        }
        ...
        if (end.length < 3) {
          _showSnackBar('Invalid end date');
          return;
        }
        DateTime enddate =
        ...
        if (difference.isNegative) {
          ...
          Navigator.push(
            context,
            MaterialPageRoute(
              builder: (context) => AddMedication4(targetUserEmail: widget.targetUserEmail),
            ),
          );
        }
      } else {
...
int _compareTimes(String timeA, String timeB) {
  ...
  final List<String> timePartsA = partsA[0].split(':');
  final List<String> timePartsB = partsB[0].split(':');
  if (timePartsA.length < 2 || timePartsB.length < 2) return 0;

  final int hourA = int.parse(timePartsA[0]);
  final int minuteA = int.parse(timePartsA[1]);
  final String periodA = partsA[1];

  final int hourB = int.parse(timePartsB[0]);
  final int minuteB = int.parse(timePartsB[1]);
